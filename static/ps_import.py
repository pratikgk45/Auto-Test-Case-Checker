...
node="<script>window.onscroll = function(){scrollFunction()};function scrollFunction(){if (document.body.scrollTop > 60 || document.documentElement.scrollTop > 60)document.getElementById('go_to_top_btn').style.display='block';else document.getElementById('go_to_top_btn').style.display = 'none';}function go_to_top_fun(){document.body.scrollTop = 0;document.documentElement.scrollTop = 0;} $(function(){$('#submit_btn').bind('click', function() {var answer = confirm('Are you sure you want to submit "+task_id+".cpp ?');if(answer){$('#submit_pop_up').html(\"<span id='close_pop_up'>&times;</span><h4>Submitting...</h4><p style='font-size:13px;'>Please <b style='color:#fa4a4a;'>Do Not</b> Reload or Close the Tab</p>\").fadeIn(1000);$.ajax({type: \"GET\",url: \"/"+contest_id+"/"+task_id+"/submit\",success: function(response) {$('#submit_pop_up').html(response).fadeIn(1000);$('#close_pop_up').on('click', function(){$('#submit_pop_up').fadeOut(500);});}});$('#close_pop_up').on('click',function(){$('#submit_pop_up').fadeOut(500);});}return false;});});</script>"+node
